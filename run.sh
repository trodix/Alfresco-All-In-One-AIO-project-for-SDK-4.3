...
build_share() {
    ...
    yes | docker-compose -f "$COMPOSE_FILE_PATH" rm -f ged-alfresco-share
    $MVN_EXEC clean package -pl ged-alfresco-share,ged-alfresco-share-docker
}

build_acs() {
    docker-compose -f "$COMPOSE_FILE_PATH" kill ged-alfresco-acs
    yes | docker-compose -f "$COMPOSE_FILE_PATH" rm -f ged-alfresco-acs
    $MVN_EXEC clean package -pl ged-alfresco-integration-tests,ged-alfresco-platform,ged-alfresco-platform-docker
}
...
prepare_test() {
    $MVN_EXEC verify -DskipTests=true -pl ged-alfresco-platform,ged-alfresco-integration-tests,ged-alfresco-platform-docker
}

test() {
    $MVN_EXEC verify -pl ged-alfresco-platform,ged-alfresco-integration-tests
}
